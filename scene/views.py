...
def delete(request):  # request是必须带的实例。类似class下方法必须带self一样  
    query_dict = request.GET
    id = query_dict.get('id')  
    re=Scene.objects.get(id=id).delete()   
    content = {
                'success': True,
                'message': '删除成功',             
            }
...
def save(request):
    ...
    Scene.objects.create(create_time=now,matchid=jsonData['matchid'],name=jsonData['name'],reserve1=jsonData['reserve1'],reserve2=jsonData['reserve2'],reserve3=jsonData['reserve3'],reserve4=jsonData['reserve4'],reserve5=jsonData['reserve5'],status=jsonData['status'],user=jsonData['user'])#括号里面是表字段=值
    content = {
                    'success': True,
                    'message': '新增成功',
                    'data':jsonData
...
def update (request):
    jsonData = json.loads(request.body.decode())
    re = Scene.objects.get(id=jsonData['id'])
    try:
        if  jsonData['matchid']:
...
def page(request):  # request是必须带的实例。类似class下方法必须带self一样   
    ...
    res1=[]
    if search:
        res1=Scene.objects.filter(name=search)
    else:
        res1=Scene.objects.filter()
        #Pagination
    total = res1.count()
